    ...
    {% for skill in skills %}
        ...
        {% endif %}
            ...
            <td class="text-right">
                ...
                {% if skill.status == 0 %}
                    <a href="{{ _p.web_self ~ '?' ~ {"action": "enable", "id": skill.id, "view": "nested"}|url_encode() }}" class="btn btn-success btn-sm" title="{{ 'Enable' }}">
                        <em class="fa fa-check-circle-o fa-fw"></em>
                    </a>
                {% else %}
                    <a href="{{ _p.web_self ~ '?' ~ {"action": "disable", "id": skill.id, "view": "nested"}|url_encode() }}" class="btn btn-danger btn-sm" title="{{ 'Disable' }}">
        ...
        {% if skill.children %}
            {{ macros.nested_skills(skill.children, _p, padd) }}
        {% endif %}
    {% endfor %}
{% endmacro %}
...
<div class="table table-responsive">
    <table class="table table-hover table-striped collaptable">
        ...
        <tbody>
            {{ macros.nested_skills(skills, _p) }}
        </tbody>
    </table>
</div>
