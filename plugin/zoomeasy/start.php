...
$urlHome = api_get_path(WEB_PLUGIN_PATH).'zoomeasy/start.php?'.api_get_cidreq();
$urlListRoom = api_get_path(WEB_PLUGIN_PATH).'zoomeasy/list.php?action=list&'.api_get_cidreq();
$urlChangeRoom = api_get_path(WEB_PLUGIN_PATH).'zoomeasy/start.php?action=remove&'.api_get_cidreq();
$urlAddRoom = api_get_path(WEB_PLUGIN_PATH).'zoomeasy/start.php?action=add&'.api_get_cidreq();
...
if ($enable) {
    if ($isAdmin || $isTeacher || $isStudent) {
        ...
        $form->addHeader($plugin->get_lang('ZoomEasyVideoConferencingAccess'));
        $form->addHidden(
            'action',
            'add'
        );
        ...
        if ($action) {
            switch ($action) {
                case 'add':
                    ...
                    $tpl->assign('form_zoomeasy', $form->returnForm());
                    break;
                case 'remove':
                    if ($idRoomAssociate) {
                        $res = $plugin->removeRoomZoomEasyCourse($idCourse, $idRoomAssociate, $idSession);
                        if ($res) {
...
$content = $tpl->fetch('zoomeasy/view/start.tpl');
