...
(function (d, f, g, c, h) {
    var e = MathJax.Extension.Help = {version: "2.5.0"};
    var b = "http://www.stixfonts.org/";
    var a = MathJax.Menu;
    var i = d.CombineConfig("HelpDialog", {
        closeImg: g.urlRev(c.imageDir + "/CloseX-31.png"),
        styles: {
            "#MathJax_Help": {
                position: "fixed",
                left: "50%",
    ...
    e.Post = function () {
        ...
        var k = window.innerHeight || l.clientHeight || l.scrollHeight || 0;
        if (a.prototype.msieAboutBug) {
            j.style.width = "20em";
            j.style.position = "absolute";
            j.style.left = Math.floor((document.documentElement.scrollWidth - j.offsetWidth) / 2) + "px";
            j.style.top = (Math.floor((k - j.offsetHeight) / 3) + document.body.scrollTop) + "px"
        } else {
            j.style.marginLeft = Math.floor(-j.offsetWidth / 2) + "px";
            j.style.top = Math.floor((k - j.offsetHeight) / 3) + "px"
        }
    };
    e.Remove = function (j) {
        if (e.div) {
    ...
    MathJax.Callback.Queue(d.Register.StartupHook("End Config", {}), ["Styles", g, i.styles], ["Post", d.Startup.signal, "HelpDialig Ready"], ["loadComplete", g, "[MathJax]/extensions/HelpDialog.js"])
